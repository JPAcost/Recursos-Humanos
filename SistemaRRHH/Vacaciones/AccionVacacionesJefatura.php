...
if (!can('Vacaciones_aprobados') && !isJefatura() && !isAdmin()) {
    $_SESSION['mensaje'] = "No tiene permisos para gestionar vacaciones en jefatura.";
...
$stmtCheck = $conexion->prepare("
    SELECT idVacaciones, estado, dias_solicitados
    FROM vacaciones
    WHERE idVacaciones = ?
    LIMIT 1
");
...
if ((int)$vacacion['estado'] !== 0) {
    $_SESSION['mensaje'] = "La solicitud ya fue procesada por jefatura.";
    $_SESSION['tipo'] = "warning";
    header("Location: AprobarVacacionesJefatura.php");
    exit();
}
...
$diasAprobados = ($accion === 'aprobar') ? (int)$vacacion['dias_solicitados'] : 0;
...
if ($stmt->execute()) {
    $_SESSION['mensaje'] = ($accion === 'aprobar')
        ? "Vacación aprobada por jefatura y enviada a RRHH."
        : "Vacación rechazada por jefatura.";
